...
- `windows\tools\` 下的六个取模工具（均只输出纯 `bin`）：
  - `img2bin_raw.exe`
  - `img2bin_imprle.exe`
  - `img2bin_rle.exe`
  - `img2bin_qoi.exe`
...
六个取模工具只输出纯 `bin`（不带信息头，不输出数组文本），`.c/.h` 由统筹管理器在上层生成，
数组内容与 `.bin` 逐字节一致。用户使用、程序集成和解码编写，都以"当前工具实际输出 +
`--info` 元数据 + 当前用户文档"为准。详细说明见 [协议与验证说明](docs/user/README-protocol.md)。
