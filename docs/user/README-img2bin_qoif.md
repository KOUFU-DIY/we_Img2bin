...
原始 QOI（无字典）压缩取模工具：只保留小差分、大差分、行程、原始块四类 op，不使用哈希字典。压缩率略逊于带字典的 `img2bin_qoi`，换来 QOI 系里最简单的解码器——零字典 RAM、零哈希计算，只需维护一组前像素状态。
...
所有 `.bin` 都以 6 字节头开始，之后紧跟 QOIF 数据流（数据流本身无任何头，也无结束码）：
...
所有比较与差分运算都在**量化后的目标格式通道值**上进行（例如 `rgb565` 用 R5/G6/B5，`argb4444` 用 A4/R4/G4/B4），不是 8 位 RGBA。

### 解码状态与初始值

只需一组前像素状态 `prev`：初始 `r=0, g=0, b=0`，`a =` 该格式 Alpha 位宽的最大值（`argb8888/argb8565` 为 255、`argb6666` 为 63、`argb4444` 为 15、`argb2222` 为 3、`ragb5155` 为 1、非 Alpha 格式恒 255）。

### op 一览（首字节区分）

| op | 名称 | 语义 |
| --- | --- | --- |
| `0x00..0x3F` | —— | **不会出现**，遇到视为损坏流 |
...
2. 否则先写出未完的 RUN，再按序尝试：Alpha 未变且可 DIFF → DIFF；Alpha 未变且可 LUMA → LUMA；Alpha 未变且该格式支持 `0xFE` → `0xFE`；其余 → `0xFF`
