...
- 宽高各占 16 位，上限 65535
- `indexqoi` 的算法 nibble 维持 `0x4` 不变：indexQOI V3（行索引 + 行去重）
  与旧版靠**索引头首字节**区分（V3 恒 `0x0F`，V2 为 `0x0E`，V1 为 `0x0D`，
  旧版均按损坏流拒绝），不占用新的算法 nibble
...
- `indexQOI` 不同行号的空降解码输出（V3 每行一个索引项）

## indexQOI（V3）的当前协议结论

当前 `indexQOI` 规则以工具实现为准：

- 文件布局：6 字节通用资源头 + 14 字节索引头 + u16/u32 行索引表
  + 静态调色盘 + QOI 数据流
- 索引头首字节 `0x0F` 兼作版本标识（V2 的 `0x0E`、V1 的 `0x0D`，
  参考解码器均按损坏流拒绝）；头内 `m16` = u16 表行数，byte7..12 保留恒 0
- **固定按行索引**：每行一个索引项（无间隔参数），第 r 行在 `r < m16` 时查
  u16 表第 r 项、否则查 u32 表第 `r − m16` 项；索引值相对 QOI 数据流起点
  （调色盘之后），不是文件起点
- **行去重**：内容完全相同的行只存一份，索引直接指向首次出现行的偏移；
  偏移不随行号单调，顺序全流解码不再成立，必须经行索引逐行解码
- 静态调色盘 0~64 项，每项一个完整原始格式像素（含 Alpha，字节序同全量像素）；
  编码器两遍法只统计存储行：净收益 > 每像素字节数才进盘，收益降序、同收益按
...
- 行首只允许 调色盘 op 或 `0xFF`，RUN 不跨行，每行空降解码自包含
- 每行解满宽度个像素即停，无尾部结束码
