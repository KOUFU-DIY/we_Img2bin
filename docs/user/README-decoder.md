...
## 九、indexQOI（V3，固定行索引 + 行去重 + 静态调色盘）解码

```text
indexQOI = 6字节通用资源头 + 14字节索引头 + u16行索引表 + u32行索引表
         + 静态调色盘 + QOI数据流
```

一句话定义：indexQOI V3 = 按行索引的 QOI + 行去重 + 静态字典（调色盘）。
每行一个索引项、每行独立解码（“通过行索引从任意行空降”），内容完全相同
的行只存一份；同时用一张编码时统计生成的全局调色盘把高频的“难压缩颜色”
压成单字节 op。解码器零 RAM 字典、零哈希计算，调色盘直接存放在图像数据
内（可原地读 flash）。
...
```text
byte0    = 头长度，当前固定 0x0F（兼作版本标识；V2 的 0x0E、V1 的 0x0D 均视为不支持）
byte1-2  = 宽度，16bit，大端
byte3-4  = 高度，16bit，大端
byte5-6  = m16 = u16 行索引表项数，16bit，大端
...
**每行一个索引项**（索引项总数 = 高度）：第 r 行在 `r < m16` 时查 u16 表
第 r 项（2 字节大端），否则查 u32 表第 `r − m16` 项（4 字节大端）。

```text
u16表起点    = 14
u32表起点    = 14 + m16×2
palette_pos = 14 + m16×2 + (高−m16)×4
...
**行去重**：内容完全相同的行不再重复存储，索引直接指向首次出现行的
偏移。解码器对去重完全无感知（照表跳转即可），但要注意两点：

- 偏移**不随行号单调递增**（回指旧行的索引比前一行的小是正常现象）
- “从头顺序解到尾”在 V3 不再成立——任何消费者都必须经行索引逐行解码

## 5. 行结构（行自包含）

- 每行独立编码：行首不依赖任何上文，行尾编码器必然已强制结束 RUN
  （RUN 不跨行）
- 行首 op 只可能是两种：**调色盘 op**（颜色在盘中）或 **`0xFF` 原始全量**，
  因此从行索引空降解码是自包含的，无状态需要复位（调色盘全局有效）
...
然后从 `cursor` 开始解码，第一个解出的像素就是第 r 行第 0 列，解满
`宽` 个像素即停（每行到数即停，数据流没有尾部结束码）。

## 7. indexQOI 和普通 QOIF 的关系

在 `QOIF` 解码器基础上仅需三处改动，无需任何 RAM 字典：

1. 初始化时从头解析出行索引表与调色盘位置（只需一次）
2. 解码循环新增一个 `op < 条目数` 的查盘分支
3. 外层改为行循环：每行查一次索引、解满宽度个像素即停
