...
仅 `img2bin_raw.exe` 支持的 Alpha 蒙版格式（只存透明度，运行时由 GUI 染色）：

- `A8`、`A4`、`A2`、`A1`

其余五个工具对 Alpha 蒙版格式的行为：显式 `--format/--formats` 点名时报 CLI 错误（退出码 1），`--formats all` 时静默滤除；它们的 `--info` 与 `--list-formats` 也不会列出这四个格式。
...
- **只有 raw 算法**（算法 nibble 恒 `0x0`），任何其他算法 nibble 与 Alpha 蒙版组合都是非法流
- **来源**：取输入图片的 Alpha 通道；无 Alpha 的输入（如 JPG）视为全不透明。编码量化为 `(a × ((1<<bpp)-1) + 127) / 255`，忽略 `--bg-color`
- **行打包**：每行从新字节开始，行字节数 = `(宽 × bpp + 7) / 8`，行尾补位填 `0`
...
- 解码按像素数到数即停，`0xA0 0x88` 用于完整性校验（不能靠扫描定位）
